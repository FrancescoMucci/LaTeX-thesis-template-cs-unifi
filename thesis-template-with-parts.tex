%--------------------------------------------------------------
%                Template for UniFI thesis (with parts)
%--------------------------------------------------------------
...
\smartClearPage
\phantomsection
\part*{Acronimi ed Elenchi di Figure, Tabelle e Codici}
% TO DO: remove the page number from the first page of unnumbered parts.

\addChapterBookmark{Acronimi}{boomark:acro}
% Although we do not include an entry for the acronyms in the table of contents, we create a bookmark for it at level 0. 

\include{1-front/acronimi}

\addChapterBookmark{Elenco delle figure}{bookmark:lof}
% Although we do not include an entry for the list of figures in the table of contents, we create a bookmark for it at level 0. 

\listoffigures
% The "\listoffigures" macro is used to request the production of a list of figures.

\addChapterBookmark{Elenco delle tabelle}{bookmark:lot}
% Although we do not include an entry for the list of tables in the table of contents, we create a bookmark for it at level 0. 

\listoftables
% The "\listoftables" macro is used to request the production of a list of tables.

\addChapterBookmark{Elenco dei codici}{bookmark:lol}
% Although we do not include an entry for the list of listings in the table of contents, we create a bookmark for it at level 0. 
...
\part{Appendici Informative}
	...
	\include{3-appendix/template/compilazione}

	\include{3-appendix/template/scrittura}

\part{Appendici di Test}

	\include{3-appendix/template/tests/basic-tests}
	
	\include{3-appendix/template/tests/float-tests}

	\include{3-appendix/template/tests/code-tests}

	\include{3-appendix/template/tests/math-tests}

	\include{3-appendix/template/tests/keywords-tests}
...
\smartClearPage
\phantomsection
\part*{Bibliografia e Indice Analitico}
% TO DO: remove the page number from the first page of unnumbered parts.
...
\addChapterBookmark{Bibliografia}{bookmark:bib}
% Although we do not include an entry for the bibliography in the table of contents, we create a bookmark for it at level 0. 

\bibliography{4-back/bibliography/biblio-example,4-back/bibliography/biblio-X,4-back/bibliography/biblio-cs-thesis-structure,4-back/bibliography/biblio-cs-knowledge,4-back/bibliography/biblio-latex}
...
\addChapterBookmark{Indice analitico}{bookmark:index}
% Although we do not include an entry for the index in the table of contents, we create a bookmark for it at level 0. 
...
\smartClearPage
\addPdfBookmark[-1]{Ringraziamenti}{bookmark:ringraziamenti}
% Although we do not include an entry for the acknowledgements in the table of contents, we create a bookmark for it at level -1.
